...
func TestMigration010FreshSchemaConstraintsAndIndexes(t *testing.T) {
	...
	).Scan(&migrations); err != nil || migrations != 12 {
		t.Fatalf("migration count/error = %d/%v, want 12", migrations, err)
	}
...
func TestMigration010UpgradesMigration009AndInstallsConditionalGuards(t *testing.T) {
	...
	if migrationsApplied != 12 || triggerCount != 39 {
		t.Fatalf(
			"upgraded migrations/triggers = %d/%d, want 12/39",
			migrationsApplied,
			triggerCount,
		)
	}
