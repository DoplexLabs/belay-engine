...
func TestMigration010FreshSchemaConstraintsAndIndexes(t *testing.T) {
	...
	).Scan(&migrations); err != nil || migrations != 23 {
		t.Fatalf("migration count/error = %d/%v, want 23", migrations, err)
	}
...
func TestMigration010UpgradesMigration009AndInstallsConditionalGuards(t *testing.T) {
	...
	if migrationsApplied != 23 || triggerCount != 107 {
		t.Fatalf(
			"upgraded migrations/triggers = %d/%d, want 23/107",
			migrationsApplied,
			triggerCount,
		)
	}
