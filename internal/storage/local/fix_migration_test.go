...
func TestMigration010FreshSchemaConstraintsAndIndexes(t *testing.T) {
	...
	).Scan(&migrations); err != nil || migrations != 14 {
		t.Fatalf("migration count/error = %d/%v, want 14", migrations, err)
	}
...
func TestMigration010UpgradesMigration009AndInstallsConditionalGuards(t *testing.T) {
	...
	if migrationsApplied != 14 || triggerCount != 43 {
		t.Fatalf(
			"upgraded migrations/triggers = %d/%d, want 14/43",
			migrationsApplied,
			triggerCount,
		)
	}
