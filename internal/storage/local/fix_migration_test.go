...
func TestMigration010FreshSchemaConstraintsAndIndexes(t *testing.T) {
	...
	).Scan(&migrations); err != nil || migrations != 13 {
		t.Fatalf("migration count/error = %d/%v, want 13", migrations, err)
	}
...
func TestMigration010UpgradesMigration009AndInstallsConditionalGuards(t *testing.T) {
	...
	if migrationsApplied != 13 || triggerCount != 39 {
		t.Fatalf(
			"upgraded migrations/triggers = %d/%d, want 13/39",
			migrationsApplied,
			triggerCount,
		)
	}
