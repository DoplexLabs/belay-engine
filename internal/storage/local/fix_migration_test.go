...
func TestMigration010FreshSchemaConstraintsAndIndexes(t *testing.T) {
	...
	).Scan(&migrations); err != nil || migrations != 15 {
		t.Fatalf("migration count/error = %d/%v, want 15", migrations, err)
	}
...
func TestMigration010UpgradesMigration009AndInstallsConditionalGuards(t *testing.T) {
	...
	if migrationsApplied != 15 || triggerCount != 52 {
		t.Fatalf(
			"upgraded migrations/triggers = %d/%d, want 15/52",
			migrationsApplied,
			triggerCount,
		)
	}
