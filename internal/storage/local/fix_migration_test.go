...
func TestMigration010FreshSchemaConstraintsAndIndexes(t *testing.T) {
	...
	).Scan(&migrations); err != nil || migrations != 16 {
		t.Fatalf("migration count/error = %d/%v, want 16", migrations, err)
	}
...
func TestMigration010UpgradesMigration009AndInstallsConditionalGuards(t *testing.T) {
	...
	if migrationsApplied != 16 || triggerCount != 58 {
		t.Fatalf(
			"upgraded migrations/triggers = %d/%d, want 16/58",
			migrationsApplied,
			triggerCount,
		)
	}
