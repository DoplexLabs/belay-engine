...
func TestMigration010FreshSchemaConstraintsAndIndexes(t *testing.T) {
	...
	).Scan(&migrations); err != nil || migrations != 24 {
		t.Fatalf("migration count/error = %d/%v, want 24", migrations, err)
	}
...
func TestMigration010UpgradesMigration009AndInstallsConditionalGuards(t *testing.T) {
	...
	if migrationsApplied != 24 || triggerCount != 110 {
		t.Fatalf(
			"upgraded migrations/triggers = %d/%d, want 24/110",
			migrationsApplied,
			triggerCount,
		)
	}
