...
1. Build or obtain the exact inspected Numbat commit for research.
2. Run one Codex or Claude artifact through `numbat scan --emit all`.
3. Capture NDJSON as a sanitized fixture.
4. Strictly validate record type, upstream schema, event type, and
   event/field co-occurrence.
5. Always assign a Belay UUIDv7; preserve upstream identity separately.
6. Minimize/redact into the candidate canonical event.
7. Persist into a temporary Local store.
8. Render fixture responses for `list_sessions` and
   `get_session_timeline`.

The spike excludes real hook installation, OTLP, cloud ingest, signing, browser
UI, and production encryption.

## Acceptance tests

1. A real Numbat invocation produces NDJSON accepted by the adapter.
...
12. Binary/source checksum matches the recorded research pin.

## Release decision still required

Belay's launch rule requires a released Numbat tag, while the inspected 0.3.0
wire behavior is not currently represented by a release tag. M0 may proceed
against the exact commit as an explicit research exception. Before production
